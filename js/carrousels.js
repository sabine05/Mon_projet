import imagesLoaded from "https://esm.sh/imagesloaded"; //importation de la bibliothèque "imagesloaded" pour surveiller le chargement des images

console.clear(); //efface la console pour une lecture plus propre des messages
...
class Raf { //class pour gérer les animations avec requestAnimationFrame
	constructor() {
		this.rafId = 0;
...
export function tilt(node, options) { //fonction tilt qui ajoute l'effet de basculement aux éléments
...
function resolveOptions(node, options) { //une fontion pour résoudre les options par défauts
	return {
...
function init() { //fonction pour initialiser le carrousel et ajouter l'effet de basculement
	...
	const buttons = {
		prev: document.querySelector(".slider--btn__prev"), //bouton précédant
		next: document.querySelector(".slider--btn__next") //bouton suivant
	};
...
function change(direction) { //fonction pour changer de diapositive
	return () => {
		let current = {
